...
describe('Cloudflare Workers Builds deployment', () => {
    ...
    it('declares a deployable Worker with assets, D1, R2, and no environment write bypass', () => {
        ...
        assert.equal(config.vars.BLOSSOM_ENABLED, undefined);
    });
    ...
    it('initializes the complete D1 schema during one-click deployment', () => {
        ...
        assert.match(read('README.md'), /database\/init\.sql/);
        assert.match(read('README_zh.md'), /database\/init\.sql/);
    });
    ...
    it('ships actionable Admin login messages for database setup failures', () => {
        ...
        assert.match(read('frontend-dist/index.html'), /app\.1e4229a6\.js\?v=blossom-admin-integration/);
    });
});
